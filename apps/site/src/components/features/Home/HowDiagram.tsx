...
export const HowDiagram: React.FC = () => {
  ...
  const containerRef = useRef<HTMLDivElement>(null);                        const iconMap = {
                          Upload: Upload,
                          Wand2: Wand2,
                          Briefcase: Briefcase,
                        };
    ...
    return () => {};
                          ...
                          >
                            ...
                            <div className="absolute inset-0 opacity-30">
                              ...
                              <div className="absolute bottom-10 right-10 w-40 h-40 bg-gradient-to-br from-emerald-400/20 to-cyan-400/20 rounded-full blur-xl animate-pulse" style={{ animationDelay: '1s' }} />
                              <div className="absolute top-1/2 left-1/3 w-24 h-24 bg-gradient-to-br from-pink-400/20 to-orange-400/20 rounded-full blur-xl animate-pulse" style={{ animationDelay: '2s' }} />
                            </div>
              ...
              <div className="text-center mb-16">
                <div className="flex items-center justify-center gap-3 text-primary mb-4">
                  ...
                  <div className="text-base font-bold tracking-wide">HOW IT WORKS</div>
                </div>
                ...
                <p className="text-muted-foreground mt-2 text-lg">AI-powered matching in three simple steps</p>
              ...
              </div>                              {/* Single Flowing Journey Animation */}
              <div className="relative min-h-[500px] mb-12 overflow-hidden">
                {/* Journey Path */}
                <div className="absolute inset-0 flex items-center justify-between px-8">
                  {/* Stage 1: Upload Zone */}
                  <div className="relative">
                    <div className={`w-64 h-48 rounded-2xl border-2 border-dashed border-blue-300 bg-blue-50/50 dark:bg-blue-950/30 flex flex-col items-center justify-center transition-all duration-1000 ${
                      activeStep >= 0 ? 'border-solid border-blue-500 bg-blue-100/70 dark:bg-blue-900/50' : ''
                    }`}>
                      <Upload className="w-12 h-12 text-blue-500 mb-3" />
                      <p className="text-sm font-medium text-blue-700 dark:text-blue-300">Upload Zone</p>
                    </div>
                  </div>

                  {/* Stage 2: Processing Zone */}
                  <div className="relative">
                    <div className={`w-64 h-48 rounded-xl bg-gray-900 border-2 border-gray-700 flex flex-col transition-all duration-1000 overflow-hidden ${
                      activeStep >= 1 ? 'border-green-500 shadow-green-500/30 shadow-lg' : ''
                    }`}>
                      {/* Terminal header */}
                      <div className="bg-gray-800 px-3 py-2 flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-300 ml-2">AI Processor</span>
                      </div>
                  ...
                  <div className="relative">
                    <div className={`w-64 h-48 rounded-2xl border-2 border-emerald-300 bg-emerald-50/50 dark:bg-emerald-950/30 flex flex-col items-center justify-center transition-all duration-1000 ${
                      activeStep >= 2 ? 'border-solid border-emerald-500 bg-emerald-100/70 dark:bg-emerald-900/50 shadow-emerald-500/30 shadow-lg' : ''
                    }`}>
                      <Briefcase className="w-12 h-12 text-emerald-600 mb-3" />
                      <p className="text-sm font-medium text-emerald-700 dark:text-emerald-300">Job Matches</p>
                    </div>
                  </div>
                </div>

                {/* Traveling Resume Document */}
                <div className={`absolute transition-all duration-2000 ease-in-out transform ${
                  activeStep === -1 ? 'left-8 top-1/2 -translate-y-1/2 translate-x-20' : 
                  activeStep === 0 ? 'left-8 top-1/2 -translate-y-1/2 translate-x-20' :
                  activeStep === 1 ? 'left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2' :
                  'right-8 top-1/2 -translate-y-1/2 -translate-x-20'
                }`}>
                  <div className={`relative transition-all duration-1000 ${
                    activeStep === -1 || activeStep === 0 ? 'w-24 h-32' :
                    activeStep === 1 ? 'w-32 h-20' :
                    'w-28 h-36'
                  }`}>
                    ...
                    <div className={`absolute inset-0 transition-all duration-500 ${
                      ...
                      <div className="w-24 h-32 bg-white dark:bg-gray-100 border-2 border-gray-300 rounded-lg shadow-xl relative overflow-hidden">
                        <div className="absolute top-2 left-2 right-2">
                          <div className="text-xs font-bold text-center mb-1 text-gray-800">RESUME</div>
                        ...
                        {/* Upload progress */}
                        <div className={`absolute bottom-0 left-0 right-0 h-2 bg-blue-500 transition-all duration-2000 ${
                    ...
                    <div className={`absolute -inset-4 pointer-events-none transition-opacity duration-300 ${
                      isAnimating ? 'opacity-100' : 'opacity-0'
                    }`}>
                      <Sparkles className="absolute top-0 right-0 w-4 h-4 text-yellow-400 animate-pulse" />
                      <Sparkles className="absolute bottom-0 left-0 w-3 h-3 text-blue-400 animate-pulse" style={{ animationDelay: '0.5s' }} />
                      <Sparkles className="absolute top-1/2 left-1/2 w-2 h-2 text-emerald-400 animate-pulse" style={{ animationDelay: '1s' }} />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center space-x-8">
                    <div className={`flex items-center gap-2 text-sm transition-all duration-500 ${
                      ...
                      <div className={`w-3 h-3 rounded-full transition-all duration-500 ${
                        activeStep >= 0 ? 'bg-blue-500 animate-pulse' : 'bg-gray-300'
                      }`}></div>
                      Upload Complete
                    </div>
                    <div className={`flex items-center gap-2 text-sm transition-all duration-500 ${
                      ...
                      <div className={`w-3 h-3 rounded-full transition-all duration-500 ${
                        activeStep >= 1 ? 'bg-green-500 animate-pulse' : 'bg-gray-300'
                      }`}></div>
                      AI Processing
                    </div>
                    <div className={`flex items-center gap-2 text-sm transition-all duration-500 ${
                      activeStep >= 2 ? 'text-emerald-600 scale-110' : 'text-gray-400'
                    }`}>
                      <div className={`w-3 h-3 rounded-full transition-all duration-500 ${
                        activeStep >= 2 ? 'bg-emerald-500 animate-pulse' : 'bg-gray-300'
                      }`}></div>
                      Jobs Found
                    </div>
                  </div>
