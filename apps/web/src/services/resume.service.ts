...
export const resumeService = {
  ...
  async deleteResume(id: string): Promise<void> {
    const response = await apiClient.delete<ApiResponse<{ message: string }>>(`/resumes/${id}`);

    if (!response.data.success) {
      throw new Error(response.data.error?.message || 'Failed to delete resume');
    }
  },

  async reanalyzeResume(id: string): Promise<Resume> {
    const response = await apiClient.post<ApiResponse<Resume>>(`/resumes/${id}/reanalyze`);

    if (!response.data.success || !response.data.data) {
      throw new Error(response.data.error?.message || 'Failed to reanalyze resume');
    }
