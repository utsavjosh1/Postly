      ...
      async (err: any, user: Express.User | false, info: any) => {
        try {
          if (err) {
            ...
            return res.redirect(
              `${config.FRONTEND_URL}/login?error=oauth_error&message=${encodeURIComponent(err.message)}`,
            );
          }
          ...
          if (!user) {
            ...
            return res.redirect(
              `${config.FRONTEND_URL}/login?error=oauth_failed&message=${encodeURIComponent(info?.message || "Authentication failed")}`,
            );
          }
          ...
          req.logIn(user, (loginErr) => {
            if (loginErr) {
              ...
              return res.redirect(
                `${config.FRONTEND_URL}/login?error=session_error`,
              );
            }
    ...
    try {
      ...
      const connections = {
        google: {
          ...
          avatar_url:
            req.user.provider === "google" ? req.user.avatar_url : null,
        },
      };
...
function isValidReturnUrl(url: string): boolean {
  try {
    ...
    return (
      parsedUrl.origin === new URL(config.FRONTEND_URL).origin ||
      url.startsWith("/")
    );
  } catch {
    return false;
